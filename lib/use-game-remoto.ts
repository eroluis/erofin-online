...
export function useGameRemoto(codigo: string) {
  ...
  const [identidadesTomadas, setIdentidadesTomadas] = useState<number[]>([])
  ...
  useEffect(() => {
    ...
    const consultar = async () => {
      try {
        ...
        if (!cancelado && Array.isArray(data.identidadesTomadas)) {
          setIdentidadesTomadas(data.identidadesTomadas)
        }
  ...
  const marcarIdentidad = useCallback(
    async (jugadorId: number) => {
      try {
        const res = await fetch(`/api/sala/${codigo}/identidad`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (!res.ok) return
        const data = await res.json()
        if (Array.isArray(data.identidadesTomadas)) setIdentidadesTomadas(data.identidadesTomadas)
      } catch {
        // Si falla, no pasa nada grave: es solo un aviso informativo, no bloquea nada.
      }
    },
    [codigo],
  )
